...
func benchMap(b *testing.B, bench bench) {
	for _, m := range [...]mapInterface{&syncmap.Map{}} {
...
func BenchmarkLoadOrStoreBalancedFunc(b *testing.B) {
	const hits, misses = 128, 128
	...
	benchMap(b, bench{
		...
		perG: func(b *testing.B, pb *testing.PB, i int, m mapInterface) {
			for ; pb.Next(); i++ {
				j := i % (hits + misses)
				if j < hits {
					if _, ok := m.LoadOrStoreFn(j, func() interface{} { return i }); !ok {
						b.Fatalf("unexpected miss for %v", j)
					}
				} else {
					if v, loaded := m.LoadOrStoreFn(i, func() interface{} { return i }); loaded {
						b.Fatalf("failed to store %v: existing value %v", i, v)
					}
				}
			}
...
func BenchmarkLoadOrStoreBigStruct(b *testing.B) {
	benchMap(b, bench{
		setup: func(b *testing.B, m mapInterface) {
			if _, ok := m.(*DeepCopyMap); ok {
				b.Skip("DeepCopyMap has quadratic running time.")
		...
		perG: func(b *testing.B, pb *testing.PB, i int, m mapInterface) {
			for ; pb.Next(); i++ {
				v, _ := m.LoadOrStore(i%100, new(BigStruct))
